...
import { generateValidationInfo, generateValidationLogHTML } from "./validationUtils";
...
export async function generateSignatureHTML(
  ...
  // Generar información de validación
  const validationInfo = await generateValidationInfo(baseHTML, userName);
  
  // Agregar el log de validación al HTML final
  const validationLogHTML = generateValidationLogHTML(validationInfo);
  
  // Insertar el log antes del cierre del último tbody
  // Buscar el último </tbody></table> y agregar antes del cierre
  const lastTbodyIndex = baseHTML.lastIndexOf('</tbody>');
  if (lastTbodyIndex !== -1) {
    const htmlWithValidation = baseHTML.slice(0, lastTbodyIndex) + 
      validationLogHTML + 
      baseHTML.slice(lastTbodyIndex);
    return htmlWithValidation;
  }
  
  // Fallback: agregar al final si no se encuentra tbody
  return baseHTML + validationLogHTML;
}
