...
export async function copyToClipboard(
  ...
  // Método 1: Intentar con Clipboard API moderna (funciona en la mayoría de navegadores modernos)
  if (navigator.clipboard && navigator.clipboard.write) {
    try {
      // Para iOS, usar un enfoque diferente con textarea
      if (isIOS) {
        // Crear un textarea temporal para iOS
        ...
        textarea.select();
        textarea.setSelectionRange(0, htmlContent.length);
        ...
        // Para otros navegadores, intentar con ClipboardItem
        try {
          ...
          // Si ClipboardItem falla, usar writeText como fallback
          console.warn("ClipboardItem not supported, using writeText:", clipboardItemError);
  ...
  // Método 2: Fallback con textarea (más compatible con iOS)
  try {
    const textarea = document.createElement("textarea");
    textarea.value = htmlContent;
    textarea.style.position = "fixed";
    ...
    // Para iOS, necesitamos hacer el textarea editable temporalmente
    if (isIOS) {
  ...
  // Método 3: Último intento - crear un div editable (para iOS)
  if (isIOS) {
    try {
      ...
      console.error("iOS-specific copy method failed:", iosError);
    }
  }
